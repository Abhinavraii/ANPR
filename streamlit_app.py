...
# Try to connect to the Arduino on the first available port
available_ports = find_ports()
if available_ports:
    port_name = available_ports[0]  # Use the first available port
else:
    port_name = "COM5"  # Default to COM5 if no ports are found
...
try:
    ...
    ser = None  # Set to None if connection fails
        ...
        if 2.5 < aspect_ratio < 8:
            ...
            if license_plate is not None:
                ...
                if plate_text in allowed_cars:
                    ...
                    if ser:
                        ser.write(b'1')  # Send signal to Arduino to rotate servo to 90 degrees
                else:
                    status = "Vehicle from outside (No entry)"
                    if ser:
                        ser.write(b'0')  # Send signal to Arduino to rotate servo to 0 degrees
